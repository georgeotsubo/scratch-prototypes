(function () {
  ...
  function syncScreenInert() {
    ...
    document.querySelectorAll('#auth-stack .auth-screen').forEach((el) => {
  ...
  function enterApp() {
    ...
    const auth = document.getElementById('auth-stack');
    if (auth) {
      auth.classList.add('is-done');
      auth.setAttribute('aria-hidden', 'true');
    }
    appFrame.classList.add('in-app');
    appFrame.classList.remove('welcome-photo');
    if (typeof window.__enterLoggedInApp === 'function') {
      window.__enterLoggedInApp();
    } else {
      var tabBar = document.getElementById('tab-bar-persistent');
      var tabBlur = document.getElementById('tab-bar-blur');
      var homeTab = document.getElementById('screen-home-tab');
      if (tabBar) tabBar.classList.remove('pre-app');
      if (tabBlur) tabBlur.classList.remove('pre-app');
  ...
  // Unlock fields that start readonly so Chrome/password managers
  // can't paint saved credentials into the demo on first paint.
  document.querySelectorAll('#auth-stack input[readonly]').forEach((input) => {
...
(function() {
  // ========== STATE ==========
  let currentScreen = 'screen-home-tab';
  let searchTerm = '';
  let locationTerm = '';
  ...
  function bookingSlotMinutes(str) {
  ...
  function syncBookingsCard() {
    ...
    var reservations = getReservations().slice().sort(function (a, b) {
      ...
      return bookingSlotMinutes(a.slotTime) - bookingSlotMinutes(b.slotTime);
    });
  ...
  window.__enterLoggedInApp = enterLoggedInApp;
  ...
  (function() {
    ...
    function renderDatePicker(selectedAbsIdx) {
      ...
      window.__vdSelectedAbsIdx = selectedAbsIdx;
      var html = '';
    ...
    var VENUE_DROPIN_PRICE = '$40';

    function generateClasses() {
      var classes = [];
      // One venue-wide drop-in price for all non-intro classes. Intro venues
      // override this with the $25/$40 pair on each card below.
      ...
      for (var i = 0; i < times.length; i++) {
        ...
        var cls = {
          ...
          disabled: isDisabled,
          absIdx: window.__vdSelectedAbsIdx || 0
        };
  ...
  (function() {
    ...
    //    style).
    //  • cdSelectedAbsIdx — the day shown in the header, footer, and
    //    checkout. Updates when the user taps a date or a time slot.
    ...
    function renderCdDatePicker(viewingAbsIdx) {
      ...
      picker.querySelectorAll('.cd-date-cell').forEach(function(cell) {
        cell.addEventListener('click', function() {
          if (wasDragging) return;
          // Date tap updates the viewed day, the booked-slot day, and the
          // header/footer copy so class detail always matches the picker.
          var absIdx = parseInt(cell.dataset.abs, 10);
          renderCdDatePicker(absIdx);
          cdSelectedAbsIdx = absIdx;
          if (window.__refreshBookingBarDate) window.__refreshBookingBarDate();
          var slotsEl = document.getElementById('cd-time-slots');
          if (slotsEl && cdLastSlot) {
            slotsEl.querySelectorAll('.cd-time-slot').forEach(function(s) {
        ...
        window.__lastGeneratedClasses.forEach(function(c) {
          ...
          if (!collected[t]) {
            collected[t] = {
              ...
              price: c.finalPrice || c.plainPrice || '$40',
    ...
    function populateClassDetail(cls) {
      ...
      // Date/slots — use the day the class was opened from (venue schedule
      // date, or the reserved day). Must be set before header/footer copy.
      var absIdx = (cls.absIdx != null) ? cls.absIdx : (window.__vdSelectedAbsIdx || 0);
      cdViewingAbsIdx = absIdx;
      cdSelectedAbsIdx = absIdx;
      updateClassHeaderMeta({ time: (cls.time || '').split(' · ')[0], instructor: cls.instructor });
      ...
      if (priceEl) {
        if (cls.finalPrice) {
          ...
          priceEl.innerHTML = '<span class="cd-booking-price-plain">' + cdFormatPrice(cls.plainPrice || '$40') + '</span>';
        }
      }
    ...
    function cdSnapshotPurchase() {
      ...
      return {
        ...
        plainPrice: (function () {
          ...
          return el ? el.textContent.trim() : '$40';
        })(),
        pin: pin ? {
          name: pin.name,
          lat: pin.lat,
